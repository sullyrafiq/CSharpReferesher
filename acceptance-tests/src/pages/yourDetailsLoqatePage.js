...
const selectors = {
  ...
  loqateField: 'input[name=loqateField]',
  address: '.address-lookup',
  loqateValidationMessage: '#loqateQuestion .warning__message',
  loqateAddress: '.address-lookup #loqateQuestion',
...
const questionCommands = {
  ...
  loqateField: inputQuestion('loqateField').set,
  address: {
    assert() {
      this.assert.elementPresent('@loqateAddress');
    },
    set() {
      return this
        .click('#loqateQuestion li:nth-child(3)')
        .api.pause(2000)
        .waitForElementVisible('.autocomplete__items')
        .click('#loqateQuestion li:nth-child(1)');
    }
  },
